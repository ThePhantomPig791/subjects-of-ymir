...
public class OdmTurbineTextureVariable implements ITextureVariable {
    @Override
    public Object get(DataContext context) {
        if (context.getEntity() instanceof Player player) {
            ...
            if (stack.getItem() instanceof OdmAttachableAddonArmorItem item) {
                if (item.getTurbine(stack).getItem() instanceof OdmComponentItem componentItem) {
    ...
    public static class Serializer implements ITextureVariableSerializer {
        ...
        public ITextureVariable parse(JsonObject json) {
            return new OdmTurbineTextureVariable();
        }

        @Override
        public void addDocumentationFields(JsonDocumentationBuilder builder) {
            builder.setTitle("ODM Turbine");
        }

        @Override
        public String getDocumentationDescription() {
            return "Returns the name of the ODM component attached in the entity's legging's turbine ODM slot";
        }

        @Override
        public ResourceLocation getId() {
            return SubjectsOfYmir.rsrc("odm_turbine");
        }
    }
}
