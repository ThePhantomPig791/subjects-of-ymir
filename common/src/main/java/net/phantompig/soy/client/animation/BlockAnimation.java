...
public class BlockAnimation extends PalladiumAnimation {
    ...
    public float getProgress(LivingEntity entity, float partialTicks) {
        float max = 0;
        var instances = AbilityUtil.getInstances(entity, SoyAbilities.BLOCK.get());
        ...
        for (AbilityInstance entry : instances) {
            float timer = ((BlockAbility) SoyAbilities.BLOCK.get()).getAnimationValue(entry, partialTicks);
